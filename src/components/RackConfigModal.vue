  ...
  <div class="modal modal-open" @click.self="$emit('close')">
    <div class="modal-box max-w-md">
      ...
      <div class="flex items-center gap-3 mb-4">
        <svg class="size-8 text-primary flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="flex-1">
          <h3 class="font-bold text-xl">Configure {{ rack.name }}</h3>
          <p class="text-sm opacity-70">Adjust rack specifications</p>
        </div>
        <button
          @click="$emit('close')"
      ...
      <div class="form-control mb-4">
        ...
        <label class="label">
          <span class="label-text-alt">
            <span v-if="hasDevices && localRuSize < maxUsedRU" class="text-error font-semibold">
      ...
      <!-- Current Usage Info -->
      <div v-if="hasDevices" class="alert alert-info">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" class="stroke-current shrink-0 w-5 h-5">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
        </svg>
        <div class="text-sm">
          <div class="font-semibold">Current Usage</div>
