<template>
  <div
    class="fixed top-4 right-4 z-10 rounded-lg shadow-lg transition-all duration-300 bg-base-100"
    :class="isExpanded ? 'w-80' : 'w-16'"
  >
    <!-- Header with Toggle -->
    <div
      class="flex items-center justify-between p-4 cursor-pointer rounded-t-lg hover:bg-base-200 transition-colors"
    ...
    >
      ...
      <div
        ...
        :key="provider.id"
        class="card bg-base-200 p-3 hover:border-primary transition-colors"
      >
      ...
      >
        ...
        <div class="space-y-1 text-xs opacity-70">
          ...
          <div
            v-if="totalPowerPortsCapacity > 0"
            class="flex items-center gap-2"
          >
            <svg
              class="w-4 h-4"
    ...
    <!-- Collapsed indicator -->
    <div
      v-if="!isExpanded && resourceProviders.length > 0"
      class="px-4 pb-4"
    >
