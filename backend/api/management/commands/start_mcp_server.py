...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if not settings.MCP_ENABLED:
            self.stdout.write(
                self.style.WARNING(
                    'MCP server is disabled. Set MCP_ENABLED=true in .env to enable it.'
                )
            )
            return
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f'Starting MCP server on port {settings.MCP_PORT}...'
            )
        )
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    'MCP server started successfully. Available tools:'
                )
            )
            ...
            self.stdout.write(
                self.style.WARNING(
                    'Make sure the mcp package is installed: pip install mcp'
                )
            )
